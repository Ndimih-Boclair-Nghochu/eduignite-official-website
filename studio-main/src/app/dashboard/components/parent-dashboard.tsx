...
import { GraduationCap, Award, ClipboardCheck, ShieldCheck, TrendingUp, Heart, Baby, Wallet, Activity, Coins, History, MapPin, Trophy, ChevronRight } from "lucide-react";
...
export function ParentDashboard() {
  const { data: childrenResp, isLoading: childrenLoading } = useMyChildren();
  const { data: gradesResp } = useGrades();

  // Real children data from API
  const children = childrenResp?.results ?? [];

  // Real recent marks from API
  const recentMarks = gradesResp?.results?.slice(0, 8).map(g => ({
    student: g.student,
  ...
  // Calculate family GPA mean
  const familyGpaMean = children.length > 0
    ? (children.reduce((sum, c) => sum + (c.annual_average ?? 0), 0) / children.length).toFixed(2)
    : "0.00";
                  ...
                  <div key={i} className="p-6 border-b last:border-0 hover:bg-accent/10 transition-colors flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-12 w-12 border-2 border-white shadow-sm ring-1 ring-accent">
                        ...
                        <AvatarFallback>{child.user?.name?.charAt(0)}</AvatarFallback>
                      </Avatar>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <Card className="lg:col-span-7 border-none shadow-xl overflow-hidden rounded-[2rem] bg-white">
          ...
          <CardContent className="p-0">
            <Table>
              <TableBody>
                {recentMarks.map((res, i) => (
                  <TableRow key={i} className="hover:bg-primary/5 transition-colors border-b last:border-0 h-16">
                    <TableCell className="pl-8 py-4">
                      <div className="space-y-0.5">
                        <p className="text-[11px] font-black text-primary uppercase leading-none">{res.student} • {res.subject}</p>
                        <p className="text-[9px] font-bold text-muted-foreground uppercase">{res.date} • {res.teacher}</p>
                      </div>
                    </TableCell>
                    <TableCell className="text-right pr-8">
                      <div className="flex flex-col items-end">
                        <span className="text-sm font-black text-primary">{res.score}</span>
                        <span className="text-[8px] font-bold uppercase text-muted-foreground italic">{new Date(res.date).toLocaleDateString()}</span>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
          ...
          <CardContent className="flex-1 pt-8 space-y-8 px-8">
            {children.length === 0 ? (
              <div className="text-center">
                <p className="text-white/60 text-sm">No children linked yet.</p>
              </div>
            ) : (
              <>
                {children.map((child, i) => (
                  <div key={i} className="space-y-3">
                    <div className="flex justify-between items-end">
                      <p className="text-xs font-black text-white uppercase">{child.user?.name}</p>
                      <p className="text-[10px] font-bold text-white/60 uppercase">75% Collected</p>
                    </div>
                    ...
                    <div className="flex justify-between text-[9px] font-black uppercase text-white/40 tracking-widest">
                      <span>Outstanding: {((1 - 0.75) * 150000).toLocaleString()} XAF</span>
            ...
            <div className="pt-4 border-t border-white/10 flex flex-col items-center text-center gap-4">
              ...
              <p className="text-[10px] text-white/60 leading-relaxed italic font-medium">
                Annual license renewals and tuition fees are synchronized with the national school registry.
              </p>
            </div>
          </CardContent>
        </Card>
